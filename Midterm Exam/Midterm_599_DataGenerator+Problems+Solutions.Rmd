---
title: "Data_Generation - Solutions - Midterm Exam"
author: "Pouria"
date: "3/3/2022"
...
for (i in 1:(n.groups/2)){
  ...
  coef1.rand <- c(rnorm(1,10,50), rnorm(1,5,15), rnorm(1,0,2)) 
  coef2.rand <- c(rnorm(1,15,80), rnorm(1,5,20), rnorm(1,0,.8)) 
  
  
  Earning1[,i] <- 90+coef1.rand[1] + Year1[,i]*(25+coef1.rand[2]) + Year1[,i]^2*(10+coef1.rand[3]) + noise1
  
  
  Earning2[,i] <- 300+coef2.rand[1] + Year2[,i]*(60+coef2.rand[2]) + Year2[,i]^2*(4+coef2.rand[3]) + noise2
}
...
- Take the intercept and slope coefficients obtained from this fit and then systematically evaluate the residual sum of squares (RSS, Eq. 3.3) around this optimal set of coefficients(range in `-/+ 10,000%` of the coefficients from the `lm()` with step size of `100` in your for-loop) to systematically create a contour plot of `RSS` against pairs of $\beta_0$ and $\beta_1$ similar to Fig. 3.2 in the book. 

- Repeat above this time for `Year` centered about its mean value (You should first do a new fit and find its intercept and slopes with `lm()`)
...
- Equation 3.8
$$SE(\hat \beta_{Year})=\frac{\sigma}{\sqrt{\sum_{i=1}^n(x_i-\bar{x})^2}}$$
...
sigma <- sqrt( sum(lm.fit1$residuals^2)/(500-2) )

SE.eq <- sigma / sqrt(sum( (Earnings.df_notPhD$Year - mean(Earnings.df_notPhD$Year) )^2 ))


SE.eq <- sigma/
  sqrt(sum( (Earnings.df$Year - mean(Earnings.df$Year) )^2 ))
...
## 2C. Compare the two fits using ANOVA
...
lm.fit.mixed <- lme(Earning ~ Year*Degree, data = Earnings.df, random= ~Year | Group)
...
Are there any groups for which the fixed effect would explain the trend in the data?
...
## 3.E Plot the residuals of the mixed-effect model (similar to 3A).

Is it different from 3A? Why or why not?


```{r}
lm.fit.mixed$residuals

Data_mixedeffect <- Earnings.df %>%
  mutate(fit = fitted(lm.fit.mixed),
         resid = residuals(lm.fit.mixed))
...
## 3B. Fit mixed effect model with quadratic term

Fit a linear mixed effect model with a baseline similar to `fit1` (`Year`, `Degree` and interactions) and a random effect of `Group`. 

```{r}
lm.fit.mixed <- lme(Earning ~ Year*Degree + I(Year^2)*Degree, data = Earnings.df, random=list( Group = pdDiag( ~Year+I(Year^2)) ) )
...
coeffs.model.mixed <- matrix(rep(0,30), nrow = 10)
coeffs.model.fixed <- matrix(rep(0,30), nrow = 10)


coeffs.model.fixed[1:5,1] <- coeffs.fixed[1]
coeffs.model.fixed[1:5,2] <- coeffs.fixed[2]
coeffs.model.fixed[1:5,3] <- 0

coeffs.model.fixed[6:10,1]<- coeffs.fixed[1]+ coeffs.fixed[3]
coeffs.model.fixed[6:10,2]<- coeffs.fixed[2]+ 0
coeffs.model.fixed[6:10,3]<- 0+ coeffs.fixed[6]

coeffs.model.mixed[1:5,1] <- coeffs.ranef[1:5,1]  + coeffs.model.fixed[1:5,1]
coeffs.model.mixed[1:5,2] <- coeffs.ranef[1:5,2]  + coeffs.model.fixed[1:5,2]
coeffs.model.mixed[1:5,3] <- coeffs.ranef[1:5,3]  + coeffs.model.fixed[1:5,3]
coeffs.model.mixed[6:10,1]<- coeffs.ranef[6:10,1] + coeffs.model.fixed[6:10,1]
coeffs.model.mixed[6:10,2]<- coeffs.ranef[6:10,2] + coeffs.model.fixed[6:10,2]
coeffs.model.mixed[6:10,2]<- coeffs.ranef[6:10,3] + coeffs.model.fixed[6:10,3]
...
f.mixed <- function(x) coeffs.model.mixed[,1] + coeffs.model.mixed[,2]*x + coeffs.model.mixed[,3]*x^2
f.fixed <- function(x) coeffs.model.fixed[,1] + coeffs.model.fixed[,2]*x + coeffs.model.fixed[,3]*x^2

coeflines.mixed <- alply(as.matrix(coeffs.model.mixed), 1, function(coef) {
  stat_function(fun=function(x){coef[1] + coef[2]*x + coef[3]*x^2}, colour="blue")
})
coeflines.fixed <- alply(as.matrix(coeffs.model.fixed), 1, function(coef) {
  stat_function(fun=function(x){coef[1] + coef[2]*x + coef[3]*x^2}, colour="grey")
})
...
for (i in 1:10){
  gg + 
    stat_function(fun=f.mixed[i,], colour="red", size=1.5) +
    stat_function(fun=f.fixed[i,], colour="blue", size=1.5) +
    scale_color_manual(values = c("black", "red"))
}
gg +
  facet_wrap(~Group) +
